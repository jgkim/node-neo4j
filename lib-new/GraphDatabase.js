...
(function() {
  var $, Constraint, Error, GraphDatabase, Index, Node, Relationship, Request, Transaction, URL, _normalizeAuth, _transform, assert, lib,
    slice = [].slice;
  ...
  module.exports = GraphDatabase = (function() {
    ...
    function GraphDatabase(opts) {
      ...
      this.auth = _normalizeAuth((ref = this.auth) != null ? ref : uri.auth);
    ...
    GraphDatabase.prototype.checkPasswordChangeNeeded = function(cb) {
      var ref;
      if (!((ref = this.auth) != null ? ref.username : void 0)) {
    ...
    GraphDatabase.prototype.cypher = function(opts, cb, _tx) {
      ...
      body = {
        statements: (function() {
          ...
          for (k = 0, len = queries.length; k < len; k++) {
            ...
            ref = query, query = ref.query, params = ref.params, lean = ref.lean;
      ...
      return this.http({
        ...
        return function(err, resp) {
          var columns, data, error, errors, i, ref, result, results;
          if (err) {
            return cb(err);
          }
          if (err = Error._fromResponse(resp)) {
          ...
          ref = resp.body, results = ref.results, errors = ref.errors;
          results = (function() {
            var k, len, results1;
            results1 = [];
            for (i = k = 0, len = results.length; k < len; i = ++k) {
              result = results[i];
              columns = result.columns, data = result.data;
              format = formats[i];
              results1.push($(data).pluck(format).map(function(row) {
                var column, j, l, len1;
                result = {};
                for (j = l = 0, len1 = columns.length; l < len1; j = ++l) {
                  column = columns[j];
                  result[column] = row[j];
                }
                if (format === 'rest') {
  ...
  _normalizeAuth = function(auth) {
    var password, passwordParts, ref, username;
    if (!auth) {
      return null;
    }
    if (typeof auth === 'string') {
      ref = auth.split(':'), username = ref[0], passwordParts = 2 <= ref.length ? slice.call(ref, 1) : [];
